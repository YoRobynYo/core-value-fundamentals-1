...
// Updated: April 2026 — Added Groq credits toggle (skip 90s wait when out of credits)
// =============================================================
//
// USAGE:
...
//
// REQUIRES:
//   GROQ_API_KEY      (primary - when credits available)
//   CEREBRAS_API_KEY  (fallback - generous free tier)
//   Ollama running locally (last resort — ollama serve)
// =============================================================
...
// --- TOGGLE THIS WHEN GROQ CREDITS ARE AVAILABLE ---
...
const CEREBRAS_MODEL   = CONFIG.aiConfig.cerebras.model;
const CEREBRAS_URL     = CONFIG.aiConfig.cerebras.url;
...
const API_LIMITS = {
  ...
  cerebras: { max: CONFIG.aiConfig.cerebras.maxDaily, used: 0, cutoff: CONFIG.aiConfig.cerebras.cutoff },
};
...
function printAPIStatus() {
  ...
  if (CEREBRAS_API_KEY) {
    console.log(`   Cerebras: ${progressBar(API_LIMITS.cerebras.used, API_LIMITS.cerebras.max, API_LIMITS.cerebras.cutoff)}`);
  }
  if (GEMINI_API_KEY) {
...
const DELAY_GROQ_RATE_LIMIT   = 90_000;   // 90s wait for Groq rate limits
const DELAY_CEREBRAS_RATE_LIMIT = 5_000;  // 5s wait for Cerebras (much higher limits)
...
async function callGroq(prompt) {
  ...
  if (isAtLimit('groq')) throw new Error('Groq 90% daily limit reached');
...
async function callCerebras(prompt) {
  ...
  if (isAtLimit('cerebras')) throw new Error('Cerebras 90% daily limit reached');
  ...
  const data = await res.json();
  if (!data.choices?.[0]?.message?.content) throw new Error('Invalid Cerebras response');
  trackUsage('cerebras');
  return cleanAIResponse(data.choices[0].message.content);
}
...
// Waterfall: Groq (if credits available) → Cerebras → Ollama
async function callAI(prompt, label = '') {
  // --- GROQ (Primary - only if credits available) ---
  if (GROQ_HAS_CREDITS && GROQ_API_KEY && !isAtLimit('groq')) {
    try {
      ...
      if (e.message.startsWith('RATE_LIMIT')) {
        console.log(`   ⚠️  Groq rate limited — waiting ${DELAY_GROQ_RATE_LIMIT/1000}s then retrying...`);
        await sleep(DELAY_GROQ_RATE_LIMIT);
        try {
          const result = await callGroq(prompt);
          printAPIStatus();
          return result;
        } catch (e2) {
          console.log(`   ⚠️  Groq failed again — falling back to Cerebras...`);
        }
      } else {
        console.log(`   ⚠️  Groq failed: ${e.message.slice(0, 80)} — falling back to Cerebras...`);
      }
    }
  } else if (!GROQ_HAS_CREDITS && GROQ_API_KEY) {
    console.log(`   ⏭️  Skipping Groq (no credits) - using Cerebras directly...`);
  }

  // --- CEREBRAS (Fallback - generous free tier, minimal delay) ---
  if (CEREBRAS_API_KEY && !isAtLimit('cerebras')) {
    try {
      console.log(`   🤖 Using Cerebras (${CEREBRAS_MODEL})...`);
      const result = await callCerebras(prompt);
      printAPIStatus();
      return result;
    } catch (e) {
      if (e.message.startsWith('RATE_LIMIT')) {
        console.log(`   ⚠️  Cerebras rate limited — waiting ${DELAY_CEREBRAS_RATE_LIMIT/1000}s...`);
        await sleep(DELAY_CEREBRAS_RATE_LIMIT);
        try {
          const result = await callCerebras(prompt);
          printAPIStatus();
          return result;
        } catch (e2) {
          console.log(`   ⚠️  Cerebras failed again — falling back to Ollama...`);
        }
      } else {
        console.log(`   ⚠️  Cerebras failed: ${e.message.slice(0, 80)} — falling back to Ollama...`);
      }
    }
  }

  // --- OLLAMA (Last resort - local, weak but free) ---
  try {
...
=== CONTEXT ONLY — DO NOT COPY THIS LANGUAGE INTO YOUR OUTPUT ===
${helperSummary}
...
=== FORCED REPLACE — DO THIS AFTER GENERATION ===
content = content.replace(/student/gi, 'child');
content = content.replace(/students/gi, 'children');
content = content.replace(/program/gi, 'code');
content = content.replace(/programming/gi, 'coding');
...
function validatePart(html, exNum, part, sectionName) {
  ...
  // JS syntax - but don't flag "let's" or "let us" in English
  if (html.includes('console.log'))  issues.push('Contains console.log');
  if (/\blet\s+[a-zA-Z_$]/.test(html)) issues.push('Contains "let" as code syntax (use SET instead)');
  if (/\bconst\s+[a-zA-Z_$]/.test(html)) issues.push('Contains "const" as code syntax (use SET instead)');
  if (/\bvar\s+[a-zA-Z_$]/.test(html)) issues.push('Contains "var" as code syntax (use SET instead)');

  // Banned words — from pipeline-config.json
    const jsKeywords = ['let', 'const', 'var', 'console'];
    for (const { word, replace, reason } of CONFIG.bannedWords) {
    const w = word.trim();
    // Skip JavaScript keywords - they're handled by regex above
    if (jsKeywords.includes(w)) continue;
...
async function main() {
  ...
  if (!GROQ_API_KEY && !CEREBRAS_API_KEY) {
    console.warn('⚠️  No API keys set — will use Ollama only');
  }
  ...
  // FIXED: Send first 3000 chars of helper content instead of just 30 lines
  const helperSummary = helperContent.slice(0, 3000);
  ...
  if (GROQ_HAS_CREDITS && GROQ_API_KEY) {
    ...
    console.log(`🔄 Fallback: Cerebras → Ollama`);
  } else {
    console.log(`🤖 Primary:  Cerebras (${CEREBRAS_MODEL})`);
    ...
    for (const part of ['a', 'b']) {
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
          ...
          let aiResult = await callAI(prompt, label);
          partHTML = aiResult;
          partHTML = partHTML.replace(/student/gi, 'child');
          partHTML = partHTML.replace(/students/gi, 'children');
          partHTML = partHTML.replace(/program/gi, 'code');
          partHTML = partHTML.replace(/programming/gi, 'coding');
